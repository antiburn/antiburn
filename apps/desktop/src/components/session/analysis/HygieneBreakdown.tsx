  ...
  return (
    <Tooltip label={<InlineHygieneTooltip check={check} />} delayMs={150}>
      <div
        ...
        tabIndex={0}
        className="-mx-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 rounded-control px-1.5 py-1 type-body transition-colors duration-[var(--duration-fast)] ease-out hover:bg-surface-hover focus-visible:bg-surface-hover"
      >
...
export function HygieneBreakdown({
  ...
  if (inlineGuidance) {
    return (
      <div
        className="session-checks-grid grid gap-x-6 gap-y-0.5"
