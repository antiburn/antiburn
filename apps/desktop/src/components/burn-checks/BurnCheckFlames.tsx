...
export function BurnCheckFlames({
  basisPoints,
  size = "row",
}: {
  basisPoints: number
  ...
  return (
    ...
    >
      ...
      >
        <FlameGraphic basisPoints={value} size={size} />
      </span>
    </Tooltip>
  )
}

export function BurnCheckFlame({ basisPoints }: { basisPoints: number }) {
  return <FlameGraphic basisPoints={basisPoints} size="single" />
}
...
function FlameGraphic({
  ...
  const flames = size === "single" ? [0] : FLAMES
  const viewWidth = size === "single" ? 24 : 104
  const width = size === "single" ? 13 : size === "summary" ? 104 : 78
  const height = size === "single" ? 13 : size === "summary" ? 24 : 18
  return (
    <svg
      aria-hidden="true"
      width={width}
    ...
    >
      <defs>
        <linearGradient id={`${id}-gradient`} x1="0" y1="1" x2="0" y2="0">
          <stop
            offset="0%"
            stopColor={
              lowBurn ? "var(--color-system-yellow-tint-val)" : "var(--color-brand-tint-val)"
            }
          />
          <stop
            offset="100%"
      ...
      <g mask={`url(#${id}-mask)`}>
        ...
        />
        {flames.map((index) => {
          const fraction = Math.max(0, Math.min(1, value / (10_000 / flames.length) - index))
          return (
            <rect
              key={index}
              x={index * 27}
