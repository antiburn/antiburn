...
describe("sessionBurnCheckPresentation", () => {
  it("uses terminal language only for complete nonempty results", () => {
    ...
    expect(passed.compactPhrases.map((phrase) => phrase.text)).toEqual(["2 Burn Checks passed"])
  ...
  it("keeps unassessed counts out of compact wording but in detail", () => {
    const value = sessionBurnCheckPresentation(
      sessionStatuses("clean", "clean", "notAssessed"),
      "ready",
    )
    expect(value.state).toBe("incomplete")
    expect(value.compactPhrases.map((phrase) => phrase.text)).toEqual(["2 Burn Checks passed"])
    expect(value.breakdownPhrases.map((phrase) => phrase.text)).toEqual([
      "2 passed",
      "1 not assessed",
    ])
  ...
  it("keeps processing separate from unavailable and unassessed counts", () => {
    ...
    expect(value.compactPhrases.map((phrase) => phrase.text)).toEqual(["1 Burn Check passed"])
...
describe("aggregateBurnCheckPresentation", () => {
  ...
  it("uses evidenceSettled to keep otherwise complete results nonterminal", () => {
    ...
    expect(value.state).toBe("incomplete")
    expect(value.headline).toBe("Burn Checks incomplete")
    expect(value.indicator.kind).toBe("segments")
  })
  ...
  it("retains results when a refresh fails", () => {
    ...
    expect(value.compactPhrases[0]?.text).toBe("1 Burn Check failed")
