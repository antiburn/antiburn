...
export type BurnCheckLifecycle =
  | "pending"
  | "processing"
  | "ready"
  | "stale"
...
export type BurnCheckScope = "sessionChecks" | "reportCategories"
export type BurnCheckOutcome = "failed" | "passed" | "unassessed"
export type BurnCheckState =
  | "allPassed"
  | "allFailed"
  | "mixed"
  | "incomplete"
...
export interface BurnCheckCounts {
  failed: number
  passed: number
  unassessed: number
}

export interface BurnCheckPhrase {
  outcome: BurnCheckOutcome | "status"
  text: string
}

export type BurnCheckIndicatorSpec =
  | { kind: "pass" }
  | { kind: "fail" }
...
function checkNoun(count: number): string {
  return count === 1 ? "Burn Check" : "Burn Checks"
}
...
function compactPhrases(counts: BurnCheckCounts): BurnCheckPhrase[] {
  ...
  if (counts.failed > 0) {
    phrases.push({
      outcome: "failed",
      text: `${counts.failed} ${checkNoun(counts.failed)} failed`,
    })
  }
  if (counts.passed > 0) {
    phrases.push({
      ...
      text:
        counts.failed > 0
          ...
          : `${counts.passed} ${checkNoun(counts.passed)} passed`,
    })
  }
  return phrases
}
...
function indicatorFor(state: BurnCheckState, counts: BurnCheckCounts): BurnCheckIndicatorSpec {
  if (state === "allPassed") return { kind: "pass" }
