...
/// Every directory a pi or node binary is looked for in: the shared GUI-safe
/// search list, plus the volta package image's own `bin`. volta's `PATH`
/// shim is a dispatcher binary, not a symlink that a realpath can follow, so
/// the package directory is searched directly.
fn candidate_dirs() -> Vec<PathBuf> {
    let volta_package_bin = format!(".volta/tools/image/packages/{PI_PACKAGE_NAME}/bin");
    cli_locator::search_dirs(&[volta_package_bin.as_str()])
}
...
mod tests {
    use super::*;
    use crate::provider_usage::live::sources::cli_locator::nvm_bin_dirs;
    use std::sync::Arc;
