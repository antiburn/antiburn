...
pub async fn open_main_window_session(app: AppHandle, target: SessionTarget) -> Result<(), String> {
    on_main_value(&app, move |app| {
        let state = app
            ...
            .ok_or_else(|| "the main-window state is unavailable".to_owned())?;
        ...
        if let Err(error) = app.emit_to(LABEL, SESSION_TARGET_EVENT, request) {
            ::tracing::debug!(event = "main_window_target_emit_failed", error = %error);
        }
        Ok(())
    })
    .await?
