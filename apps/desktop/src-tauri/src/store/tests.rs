...
#[test]
fn active_native_file_source_labels_uses_the_keyset_index() {
    let store = store();
    let connection = store.lock();
    let mut statement = connection
        .prepare(&format!(
            "EXPLAIN QUERY PLAN {ACTIVE_NATIVE_FILE_SOURCE_LABELS_SQL}"
        ))
        .unwrap();
    let plan_lines: Vec<String> = statement
...
#[test]
fn active_native_file_source_labels_matches_native_file_sessions_inside_the_window() {
    let store = store();
    store
        .upsert_sessions(&[session("in-window", 950), session("stale", 500)], &[])
        .unwrap();
    let mut wsl = session("wsl-in-window", 950);
    wsl.key.environment_key = "wsl:ubuntu".into();
    wsl.wsl_distro = Some("Ubuntu".into());
    store.upsert_sessions(&[wsl], &[]).unwrap();

    // The window is `ACTIVE_SESSION_WINDOW_SECS` wide, ending at `now`.
    let now = 950 + ACTIVE_SESSION_WINDOW_SECS - 1;
    let labels = store.active_native_file_source_labels(now).unwrap();

    assert_eq!(
        labels,
        vec!["/home/avery/.claude/projects/demo/in-window.jsonl".to_string()],
        "only the native, in-window, file-backed session's label is returned"
    );
}
