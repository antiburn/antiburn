...
impl TrayIcon {
    ...
    /// Overrides the automatic pressed highlight for this status item. **macOS only**.
    ///
    /// `Some` keeps the requested highlight through mouse events. `None` restores
    /// the standard highlight on mouse-down and clears it on mouse-up.
    #[cfg(target_os = "macos")]
    pub fn set_highlight_override(&self, highlight: Option<bool>) {
        self.tray.borrow().set_highlight_override(highlight);
    }
