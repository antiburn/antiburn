...
motion:
  ...
  overview-radial-in: "1400ms linear; the week flower's data sweeps clockwise from the reset through a conic mask (--overview-radial-sweep in overview.css); each waste pin pops (--duration-slow overshoot) as the sweep passes its time of week, a label follows its pins by --duration-slow, the config flag's pole rises first and its rows follow --duration-quick apart; plays once per app run; reduced motion clamps it and drops the delays"
  overview-weeks-apart: "600ms (2 x --duration-slow) --ease-out-quart; Break apart moves each week of the linear week chart from the shared plot into its own row with translateY + scaleY on the week group (transform-box view-box, non-scaling strokes), labels and pins move with translate only, rows start --duration-quick / 3 apart (--row in overview.css), the shared grid fades over --duration-medium; every week draws in context-stroke through --week, past weeks fade; each pin stem (.week-stem) scales with the same transition so it stays on its week's line; reduced motion drops the transition"
  overview-note: "--duration-slow --ease-out-quart; the linear week chart annotates the part in focus with a boxless note (kicker in type-metadata caps with 0.06em tracking, type-headline lede, type-callout body, 2px share bars under the top sessions), a 1px label-ink leader that runs 22px diagonally from the anchor and 16px flat (6px gap to the text), and a 3px anchor dot where no mark already shows the point; in dark mode a surface text-shadow halo keeps the text clear over lines (light mode has none, as with .week-halo); the leader draws over 1.4 x --duration-slow, the note fades in and rises 4px after 0.35 x --duration-slow; a new focus remounts the note so the entrance plays again; the week flower keeps the ui-tooltip card; reduced motion clamps it through the shared rule"
