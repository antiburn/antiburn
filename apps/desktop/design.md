...
motion:
  ...
  overview-weeks-apart: "600ms (2 x --duration-slow) --ease-out-quart; Break apart moves each week of the linear week chart from the shared plot into its own row with translateY + scaleY on the week group (transform-box view-box, non-scaling strokes), labels and pins move with translate only, rows start --duration-quick / 3 apart (--row in overview.css), the shared grid fades over --duration-medium; week colours come from existing tokens through --week and the week-tone-0..6 classes (context-stroke, token-out, mode-other, mode-talking, system-green, system-gold, system-indigo, newest first); reduced motion drops the transition"
