...
| `AmpFileChanges`               | Amp `file-changes/**/*.{json,jsonl}`                               | Not admitted because file changes are not threads                                       | No reader                             |
...
| All S sources               | S                   | Both parent and worker require an exact reviewed premium-tier identity. An unlisted model name is unknown and blocks clean; family, substring, and prefix matches do not classify it.                                                                                                                                                                                                                               |
...
| Claude and Codex            | B                   | An exact captured harness-version/model catalog entry supports scoped definition findings with complete calls. Approximate version or model matches remain display-only and cannot support a finding or clean result. Deferred, situational, and zero-cost definitions are excluded. Catalog resolution does not prove every historical enabled tool or exposure change; it cannot justify a whole-inventory claim. |
...
replacement. A cause turn count is emitted only when the model aggregate starts
after the replacement boundary. Route summaries retain only a final timestamp,
so a range that spans the boundary cannot safely provide a post-boundary count.
Token-burn percentages are unknown when a required price or the total-token
denominator is absent. The estimator does not use a 10 percent fallback and
does not force a positive minimum.
