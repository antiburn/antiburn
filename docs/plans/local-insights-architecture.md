...
- **Issue:** [#70](https://github.com/antiburn/antiburn/issues/70) — _Implement Local Insights from the reviewed architecture plan_
- **Date:** 2026-08-20

## Overview / Problem

antiburn discovers local coding-agent sessions and computes per-session metrics, but it cannot answer cross-session questions: which sessions ran too deep, which MCP servers and skills load into context and are never used, which subagents inherit a premium model, where cache tokens are resent, and where a provider quota blocked work. antiburn must answer them on the device, from the reader's own transcripts, without sending session content anywhere.

Two obstacles block that today. First, analysis reads each transcript whole (`crates/antiburn-local/src/analysis/vendors/mod.rs:read_source` calls `std::fs::read_to_string`) and materializes a complete `Vec<NormalizedEvent>`, so memory grows with transcript size — unacceptable for an always-running background utility. Second, nothing durable exists between the transcript and a report. `session_analysis` is a narrow re-pricing cache: after migration V8 it retains only `model_breakdown_json`, `inclusive_models_json`, `source_fingerprint`, and `pricing_generation` (`apps/desktop/src-tauri/src/store/model.rs:AnalysisRecord`). Full `SessionMetrics` are recomputed on demand by `apps/desktop/src-tauri/src/analysis.rs:analyze` and are never persisted. So no persisted row carries the rule-neutral facts a detector needs, and none carries a source generation or a parser revision, so no consumer can tell whether a cached row is current.

The reviewed architecture solves both with two streaming pipelines separated by compact, versioned per-session evidence in antiburn's SQLite database. This issue executes that architecture for the first provider (Claude Code) through the source plan's Phase 11.
...
`crates/antiburn-local` is the only crate under `crates/`.

**Discovery.** `crates/antiburn-local/src/discovery/mod.rs` provides `Explorers::discover_recent_sessions`, `Explorers::discover_recent_sessions_with_progress`, `Explorers::provider_db_fingerprint` (delegating to `AgentExplorer::provider_db_fingerprint`), `SessionSource::{File, Inline, ProviderDb}`, and `SessionLog`. `SessionLog` carries `agent_type`, `source`, `updated_at`, and `environment` — it has **no** `session_id` field. `SessionLog::dedupe_key`, `SessionLog::cursor_key`, and `SessionLog::source_label` identify the _discovered source_ for dedupe and incremental cursors. They are not the persisted session identity, which comes from parsed metadata, `scan.rs:recovered_id`, and `SessionKey` (see correction 7). `ACTIVE_SESSION_WINDOW_SECS` is `180` and lives in the same module. **It is a UI liveness predicate today, not a processing gate, and this plan does not make it one** (see the scan paragraph below). `SOURCE_PREVIEW_BYTES` caps bounded metadata previews at 16 MiB. `discovery/mod.rs:session_source_preview` already reads the head of every file source: `file.take(SOURCE_PREVIEW_BYTES).read_to_end(...)`.

**Analysis.** `crates/antiburn-local/src/analysis/interface.rs` defines `RawSource` as an **enum** (`Jsonl(String) | File(PathBuf) | Sqlite(PathBuf)`), `SessionInput`, and `VendorAdapter`. `VendorAdapter` has exactly two methods today: `agent()` and `normalize()`. `crates/antiburn-local/src/analysis/vendors/mod.rs:adapter_for` dispatches to six adapter statics: `claude`, `codex`, `cursor`, `opencode`, `antigravity`, and the `generic_jsonl` fallback (`jsonl.rs` and `sqlite.rs` are shared helpers, not adapters). `crates/antiburn-local/src/analysis/mod.rs` exposes `normalize_source` and `analyze_sources_with`; `crates/antiburn-local/src/analysis/engine.rs` defines `SessionMetrics` and `analyze_session`. **`SessionMetrics` has no `first_ts_ms`, no phase segments, no pattern signals, and no pattern score.** Upstream `222e575` ("Remove misleading session pattern analytics", PR #113) removed them, and `crates/antiburn-local/src/analysis/tests.rs:serialized_metrics_exclude_removed_phase_and_health_data` locks that removal by asserting `disruptionCount`, `phaseDistribution`, `patternScore`, `signals`, `segments`, and `skillUses` are absent from the serialized value, along with each bucket's `dominantPhase` and `distribution`. `analyze_session` still computes an earliest timestamp, but only as a local value folded from each event's `NormalizedEvent::ts_ms`; it is not an output field. `crates/antiburn-local/src/analysis/initial_context.rs` is a second independent pass over the same payload (`parse_initial_context`, `parse_skill_descriptions`), called from `analyze_sources_with`. `crates/antiburn-local/src/analysis/vendors/claude.rs` is the Claude adapter.
...
**Schema.** `apps/desktop/src-tauri/src/store/schema.rs` declares `MIGRATIONS: &[&str] = &[V1, V2, V3, V4, V5, V6, V7, V8]` and states the append-only rule in its module comment ("Never edit an entry that has shipped"). **The migration head is V8, so the next free number is V9.** Tables: `setting`, `scan_root`, `session`, `session_analysis`, `session_relation`, `scan_state`, `repository`, `consent_grant`, `usage_analytics_event`, `usage_analytics_identity`. No `session_evidence` table exists.

The three migrations above V5 all act on `session_analysis`. V6 is `DELETE FROM session_analysis`. V7 adds `inclusive_models_json TEXT NOT NULL DEFAULT '[]'` and deletes the rows again. **V8 reconstructs the table**: it creates `session_analysis_v8`, copies four value columns across, drops `session_analysis`, and renames. The table's final shape is therefore exactly `environment_key`, `agent`, `session_id`, `model_breakdown_json`, `inclusive_models_json`, `source_fingerprint`, `pricing_generation`. **V8 dropped `metrics_json`, `cost_json`, `active_secs`, `duration_secs`, `pattern_score`, and `analyzed_at`**, which V1 had created. `store/model.rs:AnalysisRecord` mirrors the four surviving value columns, and `Store::save_analysis` and `Store::analysis` write and read exactly those columns. There is no `metrics_json` column and no persisted full display-metrics row anywhere in the schema.

`session` is unchanged since V4. Its columns are `environment_key`, `agent`, `session_id`, `source_kind`, `source_label`, `wsl_distro`, `title`, `title_source`, `cwd`, `surface`, `updated_at_epoch`, `subagent_count`, `first_seen_at`, `last_seen_at`, plus V4's `activity_source` and `activity_cursor`.

**None of the columns this plan adds exists after V8.** `session` has no `source_fingerprint`, no `source_generation`, and no `started_at_epoch`. `session_analysis` has no `analyzed_generation` and no parser, analyzer, or metrics revision. `session_analysis.source_fingerprint TEXT NOT NULL` (the `mtime:size` string) and `pricing_generation` do already exist and are reused, never re-added.

**Desktop IPC and UI.** `apps/desktop/src-tauri/src/commands.rs` holds the `#[tauri::command]` surface, registered in `apps/desktop/src-tauri/src/lib.rs:invoke_handler` (for example `get_session_analysis`, `list_recent_sessions`, `clear_local_index`, `delete_session_data`). The frontend calls them through `apps/desktop/src/lib/ipc.ts`. `apps/desktop/src/lib/settingsPanes.ts:SETTINGS_PANE_IDS` is `general, appearance, sources, privacy, notifications, usage, about` — there is **no** `insights` pane. Panes live in `apps/desktop/src/views/settings/*Pane.tsx` under `apps/desktop/src/views/SettingsView.tsx`. Existing session UI is `apps/desktop/src/views/popover/SessionPane.tsx` and `apps/desktop/src/components/session/SessionDetailPresentation.tsx`.

**Provider usage — two existing contracts, neither one an evidence source here.** `apps/desktop/src-tauri/src/provider_usage/mod.rs` (`summarize`, `window_bounds`, `lookback_start`) derives _spend_ from `Store::usage_evidence(since_epoch)`; its module comment states that an allowance, a percentage, a remaining balance, and a reset time are not things a transcript states. Account limits live on the separate cached `dto::LiveUsageSummary` held by `apps/desktop/src-tauri/src/usage_alerts.rs:LiveUsage`, read through `LiveUsage::snapshot()`. Session evidence in this issue comes from session transcripts only, so neither contract feeds it (see **Out of Scope**).

**Fixtures and tests.** Committed transcript fixtures are `crates/antiburn-local/tests/fixtures/initial_context/{claude_realistic.jsonl, codex_realistic.jsonl, cursor_unsupported.jsonl}` and, since CH-001 shipped in PR #117, `crates/antiburn-local/tests/fixtures/claude_characterization/` — seven synthetic `.jsonl` sources, their seven goldens under `goldens/`, a `README.md`, and the harness `crates/antiburn-local/tests/claude_characterization.rs`. **Those goldens were deliberately re-baselined onto post-`222e575` behavior**, so they freeze current behavior, not pre-#113 behavior. Parser and metric tests are inline in `crates/antiburn-local/src/analysis/tests.rs` and `crates/antiburn-local/src/analysis/vendors/jsonl.rs`.

### Corrections to earlier current-code claims
...
3. **The migration head is V8, not V5 and not the count the plan assumed.** PR #60 added V5 (`usage_analytics_event`, `usage_analytics_identity`); upstream V6, V7, and V8 shipped after it, and V8 reconstructed `session_analysis` (**Current State (evidence) → Schema**). V1 through V8 have shipped and are immutable. The generation/revision migration is therefore **V9** and `session_evidence` is **V10 or later**.
4. **`session` gained `activity_source` and `activity_cursor` in V4.** Every `SessionRecord` mapping change must carry them.
5. **Provider-DB and inline sources do not merely lack an "equivalent" fingerprint — they map to `MISSING_FINGERPRINT` (`"-"`), and `analysis.rs:cache_is_fresh` rejects that value outright.** Their analysis cache can therefore never be fresh today.
6. **Two usage contracts already exist.** `Store::usage_evidence` with `provider_usage::{summarize, window_bounds}` is transcript-derived local _spend_. The cached `dto::LiveUsageSummary`, read through `usage_alerts::LiveUsage::snapshot()`, is account-level limit state. Neither is an evidence source for this issue. This is a statement of fact and carries no obligation on any seam.
7. **`SessionLog` has no `session_id`, and the persisted session id is not derived from the dedupe/source-label scheme.** `SessionLog::{dedupe_key, cursor_key, source_label}` identify a _discovered source_ for dedupe and cursor purposes. The canonical persisted id comes from `apps/desktop/src-tauri/src/scan.rs:describe_one_with_activity`, which takes `SessionMetadata::session_id` from the parsed bounded metadata, falls back to `scan.rs:recovered_id` (provider path recovery via `Explorers::recover_session_id_from_path`, then the file stem, then `SessionSource::ProviderDb::session_id`, then the `SessionSource::Inline` label), rejects an empty id, and builds the `SessionKey`. A `SourceDescriptor` must preserve that exact resolution order, not invent a second identity scheme.
8. **The storage-neutrality rule for `antiburn-local` is a design boundary held by review and by the crate's dependency list.**
...
2. **Evidence is rule-neutral fact, persisted as a versioned JSON payload in a new `session_evidence` table.** Thresholds, pricing, and catalogs then change without reparsing transcripts, and the payload keeps the surviving JSON-column convention on `session_analysis` — `model_breakdown_json` and `inclusive_models_json` — rather than adding a column per map-valued fact. V8 dropped the old `metrics_json` column, so the convention this decision follows is that surviving pair, not a persisted metrics blob.
...
10. **New schema arrives as appended migrations at the live head of `MIGRATIONS`; every shipped migration constant stays immutable.** This is the stated rule in `store/schema.rs`. CH-002 shipped **V9** (generations and revisions). **V10 shipped separately for the analytics-table rename** (`ALTER TABLE usage_analytics_event RENAME TO analytics_event;`), so it is not available to this plan. The remaining migration in this plan, `session_evidence`, is therefore **V11** — the next free number after the live head `MIGRATIONS: &[&str] = &[V1, ..., V10]`. A seam that finds the head has moved past V10 again takes the next free number instead, and never renumbers or edits anything already shipped.
...
14. **Session metrics retain bounded derived state.** The reducer caps progress cells, reorder windows, active intervals, names, skills, and efficiency state. Additive totals remain exact. Documented overflow regimes can change bucket placement, active time, and bounded detail collections.
...
16. **One fingerprint contract: stable file identity where available, byte size, high-resolution modification/change time, and a hash of a small fixed head region.** This decision is the single home of that contract; the Architecture reference's fingerprint policy and CH-002 reference it rather than restating it, and it supersedes the second-resolution `mtime:size` string `analysis.rs:fingerprint_of` computes today. The head hash costs a hash over bytes discovery already reads (`discovery/mod.rs:session_source_preview`), not a new read. **Detection envelope, stated exactly: the head hash detects changes within the hashed head region only.** It closes a same-size in-place rewrite _of that region_. It does **not** prove that any byte outside that region is unchanged, and it does not prove a prefix or a whole file is append-only. Hashing more would be the unbounded work this plan exists to avoid. Two constraints bind every seam: the hashed region is small and fixed and is explicitly **not** `SOURCE_PREVIEW_BYTES` (16 MiB); and the hash is fingerprint input only — never surfaced, logged, exported, or retained as evidence. Its size is set in CH-002.
...
- A capability that could cost the reader something is named and bounded in the PR (`CONTRIBUTING.md`).
- Migrations are append-only for any constant that has reached an installed database. Pre-release the ladder may still be edited; the constraint binds at first ship (`apps/desktop/src-tauri/src/store/schema.rs`, module comment: "Never edit an entry that **has shipped**"). This plan's remaining migration, **V11** for `session_evidence`, is appended at the live head, not squashed into an existing entry. V1 through V10 — including this plan's shipped V9 and the separately shipped V10 analytics rename — are not edited (Locked Decision 10).
- Styling work in `apps/desktop` follows the desktop design system: read `apps/desktop/design.md` first, use only the semantic utilities it documents (`bg-/text-/border-<token>`, the `type-*` scale, `rounded-control`, `duration-*`), and never hard-code a color, a type size, a radius, or a duration. `scripts/check-design-drift.mjs` runs in CI; a token or stylesheet change updates `design.md` in the same change, and a new stylesheet is added to its `sources:` list (`AGENTS.md`, "Desktop design system").
- The engine stays storage-neutral and has no project-operated service dependency.
...
- `aislop ci --changes --base origin/main` is clean. **The base is `origin/main`, always.** A worktree's local `main` ref can be very stale, so `origin/main` is the only valid base, and seam PRs target `main` (PR #117 did). The gate is **already present and passing**, not a future dependency: `.aislop/config.yml` and the required `aislop` "slop gate" job exist on `origin/main` from antiburn#89. antiburn#90 ratchets thresholds separately and is not a dependency. `complexity/file-too-large` is enforced, so a new module must stay within the configured threshold.
...
- Initial-context attribution semantics — a breakdown of named skill/MCP/built-in-tool sources for every supported agent (`claude`, `codex`), `None` only for an unsupported agent: `crates/antiburn-local/src/analysis/initial_context.rs`. The built-in-tool rows (`InitialContextTokenSource::BuiltinTool`, `source: "builtin_tool"`) come from the embedded tool catalogue (`crates/antiburn-local/src/analysis/tool_catalog.rs`, `ToolCatalog::lookup`), resolved by the harness version and model the same raw-transcript pass reads off the session (Claude: the top-level `version` field and the most frequent full `message.model` id; Codex: `session_meta.payload.cli_version` and `turn_context.payload.model`). A tool the session deferred (Claude's `deferred_tools_delta` attachment; Codex has no known marker, so every tool counts as loaded) gets `deferred: true` and `DEFERRED_TOOL_TOKEN_ESTIMATE` (5) instead of its measured cost. `fill_use_counts` fills each row's `use_count` from the session's per-raw-tool-call counts (`SessionMetrics::tool_calls_by_name`), matched case-insensitively against the row's displayed name or any of its `match_names` (an internal, never-serialized field: the catalogue name, each alias, and each alias's bare last dot-segment). A row's displayed `source_name` is itself an alias's last dot-segment, not its full namespaced spelling — Codex calls `functions.exec` and `collaboration.spawn_agent` by `exec` and `spawn_agent` in the transcript, never the dotted alias. Codex's `exec` custom-tool-call also unwraps into its nested `tools.<name>(...)` calls for tool-mix accounting (`NormalizedEvent::tools`), which would otherwise lose the wrapper's own use; `NormalizedEvent::wrapper_tool` names the wrapper separately so `tool_calls_by_name` still counts it without double-counting or perturbing `tools`.
...
The sections below preserve the researched architecture. They remain the design reference every seam works against. Where a passage described current code, this plan's **Current State (evidence)** section supersedes it.
...
The bounded-memory guarantee applies to raw source framing, normalized-record lifetime, session metrics, and report reduction. `SessionMetricsAccumulator` retains at most 540 folded progress slots plus 64 reorder entries and capped names and details. Large sessions merge facts on an active-position quantum, so continuous values can move between progress buckets. Additive totals remain exact outside documented collection caps. Provider adapter state, one evidence identity set, and batch paths can still retain data proportional to the source, as recorded in `local-insights-followups.md`.
...
The local report should preserve the complete set of reviewed Hygiene and Efficiency finding categories.
...
| **Model Overthinking**    | Claude Code, Codex, or Pi sessions using reasoning/thinking tiers above the reviewed recommended cap.                                     | Lower the tier with the provider's effort/model controls. Prompt keywords are not inferred as explicit reasoning-tier evidence.                 |
...
Antiburn should also support an explicitly local extension for subscription or quota-limit pressure. This remains separate from the nine core categories because provider limits may represent:
...
- time buckets (`engine.rs:Bucket`), including per-bucket context occupancy and compaction-boundary zeroing;
- timestamp ordering;
- per-invocation skill data (`SessionMetrics::skill_uses`, which is `skip_serializing`).

**This list is post-`222e575`.** Phase segments, phase distribution, pattern signals, and the pattern score are not in the current contract and are not reintroduced here (**Current State (evidence) → Analysis**).

Session metrics retain bounded derived facts instead of event history. The reducer keeps 540 folded progress slots, 64 reorder entries, bounded active-time intervals, and capped name and skill collections. Order-sensitive facts enter an active-position cell before cells merge, so compactions, cache misses, gaps, and mode transitions stay with their token contribution. Cell compaction can move continuous values between progress buckets. Additive totals remain exact outside documented collection caps.

This bound does not cover provider adapter state or the batch path's complete normalized-event vector. Those measured follow-ups do not require transcript archival or a disk-backed metrics spool.
...
**The maximum retained record size is 8 MiB.** It is a safety valve against one pathological record, not a transcript-size limit — there is no total file cap (Locked Decision 9). Adapters, including Pi, do not drop events after a provider-specific total record threshold. Exact metric state can retain all metric-bearing records under Locked Decision 14. Required failure behavior: drain the oversized record through its newline, mark the affected coverage `Partial` with a reason, and resume at the next record. Never truncate a record into evidence and never abort the source. Evidence for the number: measured local Claude transcripts top out near 79 KB per record, and repository precedent for bounded reads sits at 8-64 MiB (`SOURCE_PREVIEW_BYTES` 16 MiB, `antigravity.rs:HISTORY_MAX_BYTES` 16 MiB, opencode's 8/32/64 MiB caps).
...
- _Membership rule:_ a record belongs to the prefix when its **terminating newline is at or before `L`**. That is the same rule as the incomplete tail, with no special case. It is chosen for reproducibility: **given the append-only guarantee below**, `(file identity, L)` determines the evidence exactly, so reprocessing a generation is byte-identical and testable. That reproducibility is a consequence of the guarantee. It is not a property the recheck can prove on its own.
- _Nothing is lost:_ a record straddling `L` is picked up by the next generation, whose `L'` includes it.
- _Pre-read validation:_ the lifecycle has three points — version capture at discovery, `open`, then the read and its recheck. Pinning the handle covers only what happens **after** `open`. A rename between capture and `open` silently substitutes a different file, so the worker validates the opened handle against the claimed generation **before it streams any record**: stable identity, size at least `L`, and the head-hash envelope the generation recorded, every input read from that handle. The outcome of a mismatch is stated once in the **Source-validity outcomes** table above. **Replacement before pinning and rename after pinning are opposite cases:** the first is rejected and retried, the second is accepted on the original inode.
- _Pinned handle:_ the read opens the source **once** and reads `[0, L)` from that one handle. Every recheck input comes from that handle, never from the path. **Replacement after the handle is pinned is not a failure.** If the path is replaced by a rename over it mid-read, POSIX keeps the descriptor on the original inode, so the read completes on exactly the bytes the claimed generation described. That result is correct, not an error, and the replacement arrives as a new generation at the next discovery through the normal path. **Implementation trap:** an implementer who re-stats _the path_ after the read turns a harmless replacement into a spurious rejection. The recheck must use the pinned handle.
- _Recheck:_ taken from the pinned handle — size still `>= L`, and the head region unchanged when re-hashed (Locked Decision 16). An append passes and is invisible. What counts as a failed prefix recheck is stated once in the **Source-validity outcomes** table above; this bullet does not restate it.
...
- _Header rewrites:_ where a provider rewrites a header, the seam may define the prefix as `[header_end, L)` and re-read the header fresh each pass. Claude's header shape is not specified here; CH-004 carries the constraint and the per-provider note.
...
Pi adopts issue #229's recommended strict structural policy while the broader shared rollout remains open. Its inertness check reads only top-level fields, `/message` fields, and content-block shapes that the shared parser reads. Nested extension payload keys do not degrade ordinary custom rows. A custom row with direct usage, model, tool, reasoning, role, or compaction signals fails closed. The adapter never reads a `customType` value; an unsafe custom row surfaces only the closed `custom` or `custom_message` row discriminator. One framed message emits at most one unusable outcome even when it contains multiple unknown content blocks.
...
Antiburn stores no queryable session start today, and no persisted row carries one: V8 removed `metrics_json`, and `SessionMetrics` has no `first_ts_ms` field at all (**Current State (evidence) → Schema** and **→ Analysis**). `analyze_session` derives an earliest timestamp only as a local value folded from each event's `NormalizedEvent::ts_ms`. Streaming analysis must therefore populate `started_at_epoch` from trustworthy provider start metadata where the source carries it, otherwise from the earliest normalized record timestamp observed in the pass, and leave it unknown when neither exists. `first_seen_at` is discovery time and must not be used as a session-start fallback.

#### Version existing `session_analysis`

After V8 the table holds only `environment_key`, `agent`, `session_id`, `model_breakdown_json`, `inclusive_models_json`, `source_fingerprint`, and `pricing_generation` (**Current State (evidence) → Schema**). It is a re-pricing cache, not a metrics store. Add:
...
Existing `source_fingerprint` and `pricing_generation` stay and are not re-added; `source_fingerprint` remains useful diagnostic evidence. Freshness requires generation and revisions to match current values. `metrics_schema_revision` versions the metrics contract the app computes and renders. It does not imply a persisted metrics payload, and this plan adds none.
...
- Start with serde JSON, consistent with the surviving `*_json` column convention on `session_analysis` (Locked Decision 2).
...
These are the file paths the plan expected to touch. They are hints for seam carving, not a contract; verify against live code.
...
CH-001 seeded that file, and it is the sole live record of deferred work for this plan. Every entry lives there and nowhere else; this plan carries no copy to drift against it.
...
- Contributor tooling. There is no contributor-tooling scope area here. Adopting the `aislop` gate was antiburn#89, which has landed — `.aislop/config.yml` and the required `aislop` job are on `origin/main` — and ratcheting its thresholds is antiburn#90.
...
- _Maximum retained record bytes:_ **8 MiB**, with the drain-mark-`Partial`-resume failure behavior (**Architecture reference → Bounded newline framing**).
- _Concurrency and lease:_ one CPU permit, one source permit, one provider-DB permit, and a five-minute lease renewed on progress, retunable in CH-013 (**Architecture reference → Resource limits**).
- _Revision constants:_ parser, analyzer, metrics, and evidence revisions all start at **1**. Bump one when the _meaning_ of a stored value changes. Never bump for a refactor. Never bump for a catalog or pricing change — those are applied at report time precisely so they do not requeue (FR-11).
- _Insights pane and scheduling:_ opening the pane does **not** reprioritise worker scheduling. It does fire the existing on-demand `ScanController::request()` kick from a second call site (CH-012), which asks for a scan pass. That adds no mechanism and no priority ordering, and it does not change the trigger set FR-10 pins.
- _First-open experience:_ a requirement, not an open number — coverage and pending progress are shown prominently, and an incomplete report never renders as an empty or clean state (CH-012). Only the numeric backlog threshold stays with CH-012.
- _Account-level quota input:_ out of scope entirely (**Out of Scope**), with the report-time join recorded as followup entry 1.
- _Second provider after Claude:_ out of scope, recorded as followup entry 6.

## Verification Strategy & Success Metrics

**Per seam** (see **Definition of Done**): `cargo fmt`, `cargo clippy --all-targets -- -D warnings`, `cargo test` from `crates/antiburn-local` and for the desktop crate; `aislop ci --changes --base origin/main`, whose base is always `origin/main` because a worktree's local `main` ref can be very stale; frontend slices add `pnpm --filter @antiburn/desktop lint`, `type-check`, and `test`.

**End-to-end bar for the issue.**

- Golden equivalence: every currently displayed `SessionMetrics` field matches its baseline value on every characterization fixture (FR-4). The CH-001 goldens under `crates/antiburn-local/tests/fixtures/claude_characterization/goldens/` are that baseline, and they were re-baselined onto post-`222e575` behavior, so "unchanged" means unchanged against current behavior.
...
- **Schema.** Two appended migrations: the shipped **V9** (generations and revisions) and the remaining **V11** (`session_evidence`) at the live head, V10 having shipped separately for the analytics-table rename. Both are additive: new nullable or defaulted columns and one new table. No existing column is dropped or rewritten, and no shipped migration constant — V1 through V10 — is edited (Locked Decision 10). V8 already reconstructed `session_analysis` and dropped `metrics_json`, `cost_json`, `active_secs`, `duration_secs`, `pattern_score`, and `analyzed_at`; this plan neither restores nor re-drops them. A build without the new code still reads every existing row.
...
- **Deploy ordering.** CH-002's shipped V9 precedes any writer of the new columns, and CH-007's V11 table must ship before any evidence write. Both are enforced by the scope-area order.
- **Provider transcripts are never modified or deleted**, so no rollback can damage the reader's own data.

## Progress Log

Derived status lives in the records ledger, not in this document. Read it with `seams progress`.
