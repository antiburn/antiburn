...
Exact parity with the current metrics contract can require retained state proportional to the number of metric-bearing events. Some output collections, including buckets and skill uses, can themselves grow with session size. The first implementation accepts this existing unbounded metrics behavior while making it substantially less expensive: retain compact timestamp/bucket/skill facts only where exact finalization needs them, and do not retain raw text, whole JSON values, tool-output payloads, the complete transcript `String`, or complete normalized events.
...
An unknown schema variant degrades every supported top-level evidence group to `Partial` unless a structural scan proves the record inert against every evidence-bearing shape the parser reads. Inert unknowns retain `Complete` coverage because no evidence was skipped; their bounded discriminator set and count remain auditable. A present but unrecognized role, any evidence-bearing shape, a Claude `<command-name>` marker, or a non-object record fails closed for a genuinely unknown name. Known eventless records can contain command echoes and nested configuration whose scalar keys match evidence vocabulary. Those records stay inert when the parser cannot read the nested keys; shallow evidence shapes still fail closed. The existing 8 MiB record framing limit bounds the structural scan without rejecting realistic housekeeping records only because they contain many fields or deep containers. A truncated discriminator or a seventeenth distinct type also degrades the supported groups and session coverage with `CapExceeded`. Other session-scope caps continue to degrade session coverage only; changing their detector-group semantics is outside this policy. Discriminators are schema vocabulary only, never payloads, and remain capped under CH-009. Narrowing evidence-bearing loss to only affected groups is separate follow-up work.
