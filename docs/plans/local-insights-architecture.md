...
10. **New schema arrives as appended migrations after the shipped V8 head — V9 for generations and revisions, V10 for `session_evidence`; every shipped migration constant, V1 through V8, stays immutable.** This is the stated rule in `store/schema.rs`. These are the next free numbers as of `8d83005`. A seam that finds the head has moved again takes the next free numbers instead, and never renumbers or edits anything already shipped.
...
- Migrations are append-only for any constant that has reached an installed database. Pre-release the ladder may still be edited; the constraint binds at first ship (`apps/desktop/src-tauri/src/store/schema.rs`, module comment: "Never edit an entry that **has shipped**"). V9 and V10 in this plan are appended, not squashed into an existing entry, and V1 through V8 are not edited.
...
- **Schema.** Two appended migrations after the shipped V8 head: **V9** (generations and revisions), then **V10** (`session_evidence`). Both are additive: new nullable or defaulted columns and one new table. No existing column is dropped or rewritten, and no shipped migration constant — V1 through V8 — is edited (Locked Decision 10). V8 already reconstructed `session_analysis` and dropped `metrics_json`, `cost_json`, `active_secs`, `duration_secs`, `pattern_score`, and `analyzed_at`; this plan neither restores nor re-drops them. A build without the new code still reads every existing row.
