...
| Overview follow-up — remove the checks header | Implemented locally (2026-09-15): Keith asked to remove the estimated burn line, then the dial and the headline beside it. The panel is now the finding rows alone; each row still opens the full section. A state with no finding shows one closing line, and an unsettled report keeps "Still assessing N sessions" under the rows. Keith then asked for a small "Checks" label like the recent sessions header, so the panel carries the same `type-caption` label with an "All checks" link. This reverses Round 3 note 3 and Round 4's burn gauge for the Overview only; the full Burn checks report keeps its Estimated burn hero. Formatting, ESLint, type-check, Knip, all 1,655 desktop tests, and design drift pass. |
