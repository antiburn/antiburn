...
| `antiburn.analytics_opted_out`            | You turn analytics off. The event is durably queued with the current anonymous id before the preference is saved, then receives one bounded delivery attempt. If delivery fails or the app exits, it is discarded with the remaining queue and identifiers.                                                                                                                                                                                                                                                                                                                                 | No event-specific properties. It is emitted only when analytics was configured and enabled; an environment opt-out, unavailable configuration, or missing identity emits nothing.                                                                                                                                                                                                                                                                                     |
...
Settings → Privacy, then re-run the query above. Antiburn first queues one fixed
opt-out signal with the current identifier and attempts one bounded final flush.
It then discards that signal, the rest of the queue, and the identifier even if
delivery fails, so a later opt-in starts an identity that cannot be linked to
the old one.
