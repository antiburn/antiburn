...
Thirty-one fields, and this is the whole list. Fourteen are the established
...
three quota-window fields are optional and appear only on
`antiburn.quota_window_closed`. One nested resource summary appears only on
`antiburn.resource_usage_observed`. One sanitized list of record type names is
optional and appears only on `antiburn.unrecognized_records_observed`. The
...
| `properties.estimateBias`       | A closed quota window's dollars-only estimate against its last reading, signed and banded by powers of five and twenty: `low_over_20`, `low_5_to_20`, `within_5`, `high_5_to_20`, `high_over_20` (`low` means the estimate landed below the meter), or `unknown` (no reading, or no factor point). Optional and present only on `antiburn.quota_window_closed`.                                                                                                   | `within_5`                              |
| `properties.unexplainedBand`    | A closed quota window's unexplained meter rise as a share of its last reading: `none`, `under_10`, `10_to_under_30`, `30_and_over`, or `unknown` (no reading). Optional and present only on `antiburn.quota_window_closed`.                                                                                                                                                                                                                                       | `none`                                  |
| `properties.readingCoverage`    | Whether a closed quota window's last reading covered its own end: `covered` (within an hour), `stale` (an older reading exists), or `none` (no reading at all). Optional and present only on `antiburn.quota_window_closed`.                                                                                                                                                                                                                                      | `covered`                               |
...
`antiburn.session_filters_changed` carries the same fixed agent list only when
you add or remove an agent in the Sessions filters. Unknown agent slugs stay
local. Earlier releases emit `antiburn.session_filter_selected` when you select
a harness filter in the Sessions sidebar; the contextual controls do not emit
that legacy event.
...
| `antiburn.setting_toggled`               | A preference changes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `label` — one of `live_usage`, `notifications`, `launch_at_login`, `tray_icon`, `dock_icon`, `discovery_paused`, `include_non_repo_folders`. The key only; never the value.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `antiburn.limit_factor_observed`         | A background factor-learning pass produces a learned dollars-per-percent factor for a `(provider, lane)` pair. Reports the first observation for each pair in this run, then only when its `(plan, factor band, residual band)` tuple changes, and never more than once every 24 hours for the same pair even across a genuine change. Reveals a coarse sense of plan mix and estimate accuracy per provider and lane, at worst a handful of times per install per day.                                                                                                                                                      | `label` — `anthropic`, `openai`, or `google`. `detail` — `short` or `long`, the lane, or `model` for a supplemental weekly window scoped to one model, such as Claude's "Fable" limit. `plan` — the mapped plan vocabulary, including Claude's `max_5x` and `max_20x` and OpenAI's `prolite` where the provider reports those reviewed values. `factorBand` — the log-spaced dollars-per-percent band. `residualBand` — how far the meter and the estimate disagree. No dollar amount, percentage, account, model name, or timestamp.                                                                                                                                                                                                                                                  |
| `antiburn.quota_window_closed`           | The same background factor-learning pass finds a closed quota window — its reset has passed, it carries an observed `period_id`, and it closed within the last 14 days — that has not yet reported. Each qualifying window reports at most once, ever, tracked by a durable per-period marker rather than an in-run comparison. Reveals how accurate the dollars-only estimate and the shared-meter model are, per provider, lane, and plan, at worst a few times per week per install.                                                                                                                                      | `label` — `anthropic`, `openai`, or `google`. `detail` — `short` or `long`, the lane, or `model` for a supplemental weekly window scoped to one model. `plan` — the same mapped plan vocabulary `limit_factor_observed` uses, from the window's own last observation. `usageBand` — the window's meter peak, `below_80`, `80_to_under_100`, `at_limit`, or `unknown`. `bucket` — the window's count of meter regressions, bucketed. `estimateBias` — the dollars-only estimate against the window's last reading, signed and banded. `unexplainedBand` — the shared-meter model's unexplained rise as a share of the last reading. `readingCoverage` — whether a reading covered the window's own end. No dollar amount, exact percentage, account, model name, session, or timestamp. |
...
| `antiburn.burn_check_auto_fix_completed` | The confirmed Auto Fix command reaches a typed result.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `detail` — `applied_awaiting_verification`, `applied_verification_unavailable`, `recovery_needed`, `stale`, `expired`, `conflict`, `unavailable`, or `failed`. No prepared-operation ID, watch ID, setting, model, path, or private error.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `antiburn.burn_check_outcome_observed`   | `verified` fires only from an active win returned by the backend when its detector is visibly Passing and unsnoozed. Backend wins are exact current fixed cycles with a fixed verification result and verified evidence boundary. `recurred` fires only when a deliberately expanded, unsnoozed target visibly has recurrence, after that check's targets were deliberately loaded; recurrence does not require the category to remain Passed. Background verification, hidden rows, and snoozed checks emit nothing.                                                                                                        | `detail` — `verified` means the visible Passed transition; `recurred` means a visible regression. `origin` — `passive` or `action`, which describes how the independent watch started and does not claim causation. No finding, action, watch, cycle, reference, sample, or evidence revision; no exact tokens or costs.                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `antiburn.session_filter_selected`       | Legacy releases: the Sessions sidebar filter changes to a different selection. Fires only on an actual change; restoring the persisted filter when the window opens emits nothing.                                                                                                                                                                                                                                                                                                                                                                                                                                           | `label` — `notable`, `material`, `agent`, `failing`, `passing`, or `all`. `detail` — the selected harness slug, only when `label` is `agent` and the harness is one antiburn's fixed agent list recognizes; a harness this build does not recognize sends `label` `agent` with no `detail`. No session, title, repository, or count.                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `antiburn.session_filters_changed`       | A deliberate change in the contextual Sessions filters. One event per changed gesture, including Clear all and All agents; none for restores, refreshes, or no-ops. Records intent, not persistence success.                                                                                                                                                                                                                                                                                                                                                                                                                 | `label` — `agent_added`, `agent_removed`, `agents_all`, `result_failing`, `result_passing`, `result_all`, `spend_notable`, `spend_material`, `spend_all`, or `cleared_all`. Optional `detail` — a known agent slug, only for agent add/remove. No raw filter string, selected-agent array, session identity, count, cost, or threshold.                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `antiburn.provider_incidents_observed`   | Settings → Insights returns an assessed provider-incidents section, once per incident kind whose bucketed hit count differs from the last one reported for that kind during this run.                                                                                                                                                                                                                                                                                                                                                                                                                                        | `label` — `capacity`, `server_error`, or `connection`. `bucket` — deduplicated hits for that incident kind. No model name, session identifier, or message text.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `antiburn.provider_incidents_ingested`   | The background evidence worker publishes a session whose transcript gained an incident in the last two hours that its previously published evidence did not carry. One event per incident kind per publish. A re-parse of unchanged evidence, an incident older than two hours, and a session opened in the UI all emit nothing.                                                                                                                                                                                                                                                                                             | `label` — the agent that recorded the session, the same fixed list `session_opened` uses. `detail` — `capacity`, `server_error`, `connection`, `rate_limit`, or `usage_limit`. `bucket` — bucketed count of new incidents. No model name, session identifier, timestamp, path, or message text.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `antiburn.navigation_history_moved`      | Back or Forward successfully restores a main-window destination. Disabled controls and history boundaries emit nothing.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `label` — `back` or `forward`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
report exposure, state, or watch outcomes. Verified events come only from the
backend's active wins after the visible report confirms that the detector is
Passing and unsnoozed; target-row data cannot create a verified event. Each
`verified` or `recurred` plus `passive` or `action` tuple is reported at most once
per visible exposure, so multiple targets, cycles, and refreshes collapse to the
same coarse event. The workspace can emit at most one view, four distinct
surface states, four watch outcome tuples, and the events caused by deliberate
action attempts in one exposure. Refreshes and background verification are not
engagement.
...
factor and its residual to bands. Every model-scoped weekly lane, such as
Claude's supplemental "Fable" window, reports `detail=model`; the model name
and its slug never travel with the event. A
...
`antiburn.quota_window_closed` shares that pass's trigger, but answers a
different question: not the _current_ factor, but how well the factor and the
shared-meter model explained one _finished_ window after the fact. For every
`(provider, account, lane)` the pass touched, it looks back at most 14 days
for a window whose reset has already passed, that carries an observed
`period_id` — never a cadence-extrapolated or turn-gap-inferred one — and that
has not yet reported. A durable per-period marker, not an in-memory
comparison, enforces "at most once, ever": once a window's marker is written,
no later pass can report it again, even after the app restarts. The account
