...
Claude Code, Codex, and Pi resume only from complete JSONL record boundaries.
An incomplete tail keeps the current pass partial and produces no successor
snapshot. The next scan starts from a fresh full read. Resumed reads hash the
complete accepted prefix without retaining it, so a middle-of-file rewrite cannot
reuse stale parser, metrics, or evidence state.
...
| `AmpFileChanges`               | Amp           | `file-changes/**/*.{json,jsonl}`                                           | Not admitted for session discovery                                                                                                                             | File changes only                                                                                                                                                                                                     | Not a session                                                                                     |
...
parser/analyzer/evidence/coverage/resume revisions are 32/23/18/4/7. Existing
