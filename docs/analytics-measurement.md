...
| Do later Passed transitions and recurrences become visible, and did their watch start passively or from an action? Improve verification coverage without claiming causation. | Distinct reporting installations with each `(detail, origin)` tuple among deliberate Burn Checks exposures. Use exposed installations as the denominator. Never use background transitions as visits.                    | `burn_check_outcome_observed`: `detail=verified` records a visible Passed transition; `detail=recurred` records a visible regression. The candidate must belong to an active, unsnoozed Passed detector. `origin=passive` or `action` describes how the watch started and does not claim causation. Verified comes from the visible aggregate summary or an expanded target. Recurred comes only from a deliberately expanded visible target. | Background evaluation, hidden sections, snoozed checks, non-Passed results, and refresh callbacks emit nothing. Deduplicate the coarse `(detail, origin)` tuple per exposure, without sending target or remediation-cycle identifiers. Maximum: four events per exposure, independent of finding count. |
