...
export function checkDesignDrift(io = fileSystemIo()) {
  ...
  if (!Object.keys(map).length) failures.push('No semantic color aliases found');
  ...
  for (const theme of ['light', 'dark']) {
    ...
    for (const [name, ref] of Object.entries(map)) {
      ...
      if (forced === undefined) {
        failures.push(`color \`${name}\`: --color-${ref} is not set in the ${theme} palette`);
      }
      if (followed === undefined) {
        failures.push(`color \`${name}\`: --color-${ref} is unset in the ${theme} system palette`);
      }
      if (
        forced !== undefined &&
        followed !== undefined &&
  ...
  for (const match of css.matchAll(/\.type-([\w-]+)\s*\{([\s\S]*?)\}/g)) {
    if (/line-height:/.test(match[2])) {
      failures.push(`typography \`${match[1]}\`: .type-${match[1]} sets its own line-height`);
    }
  }

  // Native window materials cannot read CSS custom properties.
  const radius = /--radius-popover:\s*([\d.]+)px\s*;/.exec(css)?.[1];
  if (radius === undefined) failures.push('--radius-popover is missing or is not a pixel value');
  for (const source of WINDOW_CORNER_SOURCES) {
    ...
    const rust = pattern.exec(stripComments(io.read(source.path)).replace(/\/\/[^\n]*/g, ''));
    if (!rust) {
      failures.push(`${source.path} declares no ${source.constant}`);
    } else if (radius !== undefined && parseFloat(rust[1]) !== parseFloat(radius)) {
      failures.push(
